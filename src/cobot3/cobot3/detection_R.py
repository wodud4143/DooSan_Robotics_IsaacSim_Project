...
T_MOUNT_CAMERA = np.array([
    [-2.49271870e-04, -2.06570338e-04, -9.99999948e-01,  3.99942234e-02],
    [ 1.64938878e-06, -9.99999979e-01,  2.06569933e-04,  1.89679707e-08],
    [-9.99999969e-01, -1.59789662e-06,  2.49272206e-04, -1.15000504e-02],
    [ 0.0,             0.0,             0.0,             1.0]
])
class RGBYOLOViewer(Node):
    def __init__(self):
        super().__init__('yolo_image_window_R')

        self.create_subscription(Image, '/rgb_R', self.image_callback, 10)
        self.create_subscription(Image, '/depth_R', self.depth_callback, 10)
        self.create_subscription(TFMessage, TF_TOPIC, self.tf_callback, 10)

        self.box_exist_pub = self.create_publisher(Int32, '/box_exist_R', 10)
        self.barcode_exist_pub = self.create_publisher(Int32, '/barcode_exist_R', 10)

        self.box_coordinate_left_pub = self.create_publisher(Point, '/box_coordinate_left_R', 10)
        self.box_coordinate_center_pub = self.create_publisher(Point, '/box_coordinate_center_R', 10)
        self.box_coordinate_right_pub = self.create_publisher(Point, '/box_coordinate_right_R', 10)
        ...
        # /rgb_R, /depth_R publisher가 2개 떠도 640x640 영상만 사용
        self.expected_width = 640
        self.expected_height = 640
        self.window_name = 'YOLO Detection R'
        cv2.namedWindow(self.window_name, cv2.WINDOW_NORMAL)
        cv2.resizeWindow(self.window_name, self.expected_width, self.expected_height)
    ...
    def image_callback(self, msg):
        try:
            ...
            cv2.imshow('YOLO Detection R', display_frame)
...
#         self.get_logger().info('YOLO RIGHT compressed camera node started')
#         self.get_logger().info('Subscribed RGB   topic: /rgb_R/compressed')
#         self.get_logger().info('Subscribed Depth topic: /depth_R/compressed')
#         self.get_logger().info(f'Subscribed TF topic: {TF_TOPIC}')
...
#     def compressed_rgb_to_bgr(self, msg: CompressedImage):
#         np_arr = np.frombuffer(msg.data, dtype=np.uint8)
#         frame = cv2.imdecode(np_arr, cv2.IMREAD_COLOR)

#         if frame is None:
#             raise RuntimeError('RGB JPEG decode failed')

#         # cv2.imdecode(..., IMREAD_COLOR)는 BGR 8UC3로 반환함
#         return frame

#     def compressed_depth_to_meter(self, msg: CompressedImage):
#         np_arr = np.frombuffer(msg.data, dtype=np.uint8)
...
#         # PNG depth가 HxWx1 형태로 들어오는 경우 HxW로 정리
#         if depth.ndim == 3:
#             depth = depth[:, :, 0]

#         # 이전 압축 노드에서 depth는 PNG 저장을 위해 보통 uint16 mm로 변환됨
#         if depth.dtype == np.uint16:
#             return depth.astype(np.float32) / 1000.0

#         # 혹시 32FC1을 그대로 압축한 경우 대비
#         if depth.dtype == np.float32:
#             return depth.astype(np.float32)

#         # mono8 같은 예외 케이스. 정확한 거리값으로 쓰기엔 부정확할 수 있음.
...
#         try:
#             depth = self.compressed_depth_to_meter(msg)
...
#                 f'Depth compressed callback failed: format={msg.format}, error={e}'
#             )
...
#         try:
#             frame = self.compressed_rgb_to_bgr(msg)
...
#         except Exception as e:
#             self.get_logger().error(
#                 f'YOLO compressed detection failed: format={msg.format}, error={e}'
#             )
